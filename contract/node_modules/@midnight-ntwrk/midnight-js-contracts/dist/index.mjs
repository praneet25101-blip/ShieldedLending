...
import { makeContractExecutableRuntime, Transaction, asEffectOption, asContractAddress, exitResultOrError, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
import 'fs';
import 'path';
...
import { ImpureCircuitId, VerifierKey } from '@midnight-ntwrk/compact-js/effect/Contract';
import { ZswapOutput, ZswapTransient, ZswapInput, ZswapOffer, ContractState, Intent, ContractCallPrototype, communicationCommitmentRandomness, ContractDeploy } from '@midnight-ntwrk/ledger-v7';
...
class TransactionContextImpl {
    ...
    [MergeUnsubmittedCallTxData](circuitId, callData, privateStateId) {
        ...
        const [zswapChainState] = callData.private.unprovenTx.guaranteedOffer
            ? this.currentStates.zswapChainState.tryApply(ZswapOffer.deserialize('pre-proof', callData.private.unprovenTx.guaranteedOffer.serialize()))
...
async function createUnprovenCallTxFromInitialStates(zkConfigProvider, options, walletEncryptionPublicKey) {
    ...
    const initialPrivateState = 'initialPrivateState' in options ? options.initialPrivateState : undefined;
    const args = ('args' in options ? options.args : []);
    const exitResult = await contractRuntime.runPromiseExit(contractExec.circuit(ImpureCircuitId(options.circuitId), {
...
async function createUnprovenCallTx(providers, options, transactionContext) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(providers.zkConfigProvider, createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.getCoinPublicKey(), getNetworkId()), contractState, zswapChainState, privateState), providers.walletProvider.getEncryptionPublicKey());
    }
...
contractAddress, privateStateId) => {
    assertIsContractAddress(contractAddress);
    return ContractExecutable.make(compiledContract).getImpureCircuitIds().reduce((acc, circuitId) => ({
        ...acc,
        [circuitId]: (...args) => {
...
async function findDeployedContract(providers, options) {
    ...
    const verifierKeys = await providers.zkConfigProvider.getVerifierKeys(ContractExecutable.make(compiledContract).getImpureCircuitIds());
